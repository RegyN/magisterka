...
public class Chromosome implements Comparable<Chromosome>{
	...
	/**
	 * amount of steps allowed for the naive agents to sit around
	 */
	 ...
	 * Otherwise, it will check to see if a do nothing agents dies within the first 40 steps of playing.
	...
	public void calculateFitness(long time) {
		...
		if(constrainFitness < 0.7) {
			...
			//Play the game using the best agents
			double score = -200;
			...
			for(int i=0; i<SharedData.REPETITION_AMOUNT; i++){
				...
				// gather all unique interactions between objects in the naive agents
			...
			//playing the game using the naive agents
			for(int i=0; i<SharedData.REPETITION_AMOUNT; i++){
				StateObservation tempState = stateObs.copy();
				...
				// gather all unique interactions between objects in the best agents
...
//			else {
	 ...
	 * @param agent		current agents to play the level
	 * @return			the number of steps that the agents stops playing after (<= steps)
	 */
